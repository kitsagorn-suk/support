<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Agent_Add.aspx.cs" Inherits="Support_Project.Menu_Management_Main.Agent_Add" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active txtMem" set-lan="text:Add Agent"></li>
        </ol>
    </nav>
    <div class="row">
        <div class="col-md-8">
    ...
    <div class="section-group mb-4 col-6">
        ...
        <div class="form-row">
            <div class="col-12" style="text-align: center;">
                <button id="bAdd" class="btn btn-yellow font-weight-bold" onclick="SaveAgent()" type="button" set-lan="text:Add Agent"></button>
            </div>
        </div>
    </div>
    <asp:UpdatePanel ID="UpdatePanel2" runat="server">
        <ContentTemplate>
            <asp:Button ID="btnAddAgent" class="btn btn-lg btn-yellow font-weight-bold btn-block" OnClick="AddAgent_click" runat="server" Text="Add" Style="display: none;" />
        </ContentTemplate>
    </asp:UpdatePanel>
    <!-- Modal -->
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainScript" runat="server">
    ...
    <script>
        ...
        $(document).ready(function () {
            $("#menuManagementMain , #menuManagementMain > a , #menuManagementAgentAdd > a").addClass("active");
        ...
        function SaveAgent() {
            ...
            if ($('#<%=Name.ClientID%>').val() == "") {
            ...
            else if (document.getElementById("<%=ddlCompany.ClientID%>").value == "" && $("#<%=ShareIDLogin.ClientID%>").val() == "0") {
                ...
                if ($("#<%=ShareIDLogin.ClientID%>").val() != "0") {
                    $("#<%=CompanyAdd.ClientID%>").val($("#<%=ShareIDLogin.ClientID%>").val());
                }
                else {
                    $("#<%=CompanyAdd.ClientID%>").val(document.getElementById("<%=ddlCompany.ClientID%>").value);
                }

                $("#<%=btnAddAgent.ClientID%>").click();
            }
            SetLan(localStorage.getItem('Language'));
        }
        ...
        function alertModal(txtAlert) {
            ...
            if (txtAlert == "Add new agent success.") {
                setTimeout(function () { window.location.href = "../Menu_Management_Main/Agent.aspx"; }, 1500);
            }
            else {
