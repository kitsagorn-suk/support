<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Feedback_Add.aspx.cs" Inherits="Support_Project.Menu_Feedback.Feedback_Add" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active txtMem" set-lan="text:Add Feedback"></li>
        </ol>
    </nav>
    <div class="row">
        <div class="col-md-8">
            <h1 set-lan="text:Add Feedback"></h1>
        </div>
    </div>
    <div class="section-group mb-4 col-6">
        <div class="form-row">
            <div class="form-group col-12">
                <div class="form-group row">
                    ...
                    <div class="col-9">
                        <asp:TextBox ID="title" class="form-control" autocomplete="off" runat="server" MaxLength="150" />
                    </div>
                </div>
                <div class="form-group row">
                    <label for="username" class="col-3 col-form-label" style="text-align: right;" set-lan="html:Message__"></label>
                    <div class="col-9">
                        <asp:TextBox runat="server" ID="MessageText" TextMode="MultiLine" Rows="3" class="form-control" autocomplete="off" MaxLength="255" onkeypress="return maxLengthfield(this,255);" />
                    </div>
                </div>
            </div>
        </div>
        <div class="form-row">
            <div class="col-12" style="text-align: center;">
                <button id="bAdd" class="btn btn-yellow font-weight-bold waves-effect waves-light" onclick="SaveFeedback()" type="button" set-lan="text:Add Feedback"></button>
            </div>
        </div>
    </div>
    <asp:UpdatePanel ID="UpdatePanel2" runat="server">
        <ContentTemplate>
            <asp:Button ID="btnAddFeedback" class="btn btn-lg btn-yellow font-weight-bold btn-block" OnClick="AddFeedback_click" runat="server" Text="Add" Style="display: none;" />
        </ContentTemplate>
    </asp:UpdatePanel>
    <!-- Modal -->
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainScript" runat="server">
    ...
    <script>
        ...
        $(document).ready(function () {
            $("#menuFeedback , #menuFeedback > a , #menuFeedbackAdd > a").addClass("active");
        ...
        function SaveFeedback() {
            $("#myModalLoad").modal();
            if ($('#<%=title.ClientID%>').val() == "") {
                document.getElementById("lbAlert").setAttribute("set-lan", "text:missing 'Title' field");
                SetLan(localStorage.getItem("Language"));
                $('#modalAlert').modal('show');
                $("#myModalLoad").modal('hide');
            }
            else if ($('#<%=MessageText.ClientID%>').val() == "") {
                document.getElementById("lbAlert").setAttribute("set-lan", "text:missing 'Message' field");
                SetLan(localStorage.getItem('Language'));
                $('#modalAlert').modal('show');
                $("#myModalLoad").modal('hide');
            }
            else {
                $("#<%=btnAddFeedback.ClientID%>").click();
            }
            SetLan(localStorage.getItem('Language'));
        }
        ...
        function alertModal(txtAlert) {
            ...
            if (txtAlert == "Add new feedback success.") {
                setTimeout(function () { window.location.href = "../Menu_Feedback/Feedback.aspx"; }, 1500);
            }
            else {
